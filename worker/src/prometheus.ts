...
export class PrometheusService {
  ...
  private async eventToMetrics(
    ...
    // Event counter
    metrics.push(
      await this.createMetric("openqoe_events_total", baseLabels, 1, timestamp),
    );
    ...
    switch (event.event_type) {
      ...
      case "playing":
        ...
        if (event.data?.resolution) {
          ...
          const resolutionLabel = this.getResolutionLabel(
        ...
        if (event.data?.quartile) {
          ...
          metrics.push(
            await this.createMetric(
              ...
              await this.cardinalityService.applyGovernanceToLabels(
                quartileLabels,
              ),
              1,
              timestamp,
   ...
   */
  private async extractBaseLabels(
    event: BaseEvent,
  ...
  async sendToPrometheus(timeSeries: PrometheusTimeSeries[]): Promise<void> {
    ...
    if (prometheusConfig.username && prometheusConfig.password) {
      const auth = btoa(
        `${prometheusConfig.username}:${prometheusConfig.password}`,
      );
  ...
  convertToMimirTimeSeries(
    ...
    return timeSeries.map((ts) => {
      const Uint8ArrayTypelabels: MimirDef.cortexpb.ILabelPair[] =
        ts.labels.map((label) => {
          return {
            name: textEncoder.encode(label.name),
  ...
  private encodeRemoteWriteRequest(
    ...
    let message = MimirDef.cortexpb.WriteRequest.create({
      timeseries: timeSeries,
    });
    let buffer = MimirDef.cortexpb.WriteRequest.encode(message).finish();
   ...
   */
  private getResolutionLabel(resolution: {
    width?: number;
    height?: number;
  }): string {
