...
export class BatchManager {
  ...
  flushSync(): void {
    ...
    this.logger.info(`Flushing batch synchronously: ${eventsToSend.length} events`);

    // Use sendBeacon if available
    if (typeof navigator !== 'undefined' && navigator.sendBeacon) {
      // The actual sending will be handled by the transport layer
      // For now, just call the callback synchronously
      this.flushCallback(eventsToSend).catch(err => {
        this.logger.error('Sync flush failed:', err);
      });
    }
  }
