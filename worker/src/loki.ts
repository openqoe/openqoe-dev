...
import { BaseEvent, LokiStream, LokiPushRequest } from './types';
...
export class LokiService {
   ...
   */
  private async extractLabels(event: BaseEvent): Promise<Record<string, string>> {
  ...
  async sendToLoki(streams: LokiStream[]): Promise<void> {
    ...
    try {
      ...
      const totalEntries = streams.reduce((sum, stream) => sum + stream.values.length, 0);
      console.log(`Sent ${totalEntries} log entries in ${streams.length} streams to Loki`);
    } catch (error) {
      clearTimeout(timeoutId);
  ...
  async query(query: string, limit: number = 100): Promise<any> {
    ...
    // Build query URL
    const queryUrl = new URL(lokiConfig.url.replace('/loki/api/v1/push', '/loki/api/v1/query_range'));
